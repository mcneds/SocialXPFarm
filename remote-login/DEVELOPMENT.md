...
| Java command policy | Correct active request, stale process/context, expiry, unknown actions, disabled automation, duplicate start during sign-in, cancellation allowed only during phone sign-in, secure configuration. |
...
Commands are limited to `login` and `cancel`, and contain their own ID, process ID, authentication context and expiry. Both sides validate lifecycle state; a command acknowledged after cancellation cannot revive the cancelled operation. Test new protocol fields on both sides using the shared fixture. The companion does not expose a shell, arbitrary game commands, account-switching API, or public HTTP UI.
