...
| Microsoft password change, revoked session, MFA/account selection, OAuth failure | Complete the account's legitimate authentication flow. | Pairing opens once when silent renewal needs interaction. Network/service failures retry every 60 seconds. Rotated refresh tokens are saved before downstream Xbox/Minecraft requests. |
