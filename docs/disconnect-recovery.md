...
| Expired/invalid login session or unverified username | Renew credentials once, then reconnect using the new session. | Auth Me handoff implemented. Browser login may require intervention. Cancelled/offline login leaves retries paused. |
...
| Microsoft password change, revoked session, MFA/account selection, OAuth failure | Complete the account's legitimate authentication flow. | Auth Me UI remains open. No stored Microsoft passwords, token scraping, or repeated browser-window loop. |
...
- **Authentication:** the installed Auth Me API does not make browser authentication fully unattended. A true background renewal solution would require a separately designed OAuth integration that owns a refresh token with appropriate persistence and revocation handling. It is not implemented here.
